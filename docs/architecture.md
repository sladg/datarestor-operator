...
| -------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `backup.autorestore.com/manual-backup`       | `<Custom-Backup-Name>` | Triggers a one-time backup. The value of the annotation is used as the name for the resulting `ResticBackup` resource. If the value is set to `"true"` or `"now"`, a default name will be generated. This annotation can be applied to a **PVC** (for a single backup) or a **BackupConfig** (for all matching PVCs). The operator removes the annotation after triggering the process. |
| `backup.autorestore.com/restore-from-backup` | `<ResticBackup-Name>`  | Triggers a restore to the annotated PVC from the specified `ResticBackup`. The operator will create a `ResticRestore` resource and then remove this annotation.                                                                                                                                                                                                                         |
